<template>
  <div class="pageContent">
              ...
              <template slot-scope="scope">
                <div>
                  <el-button
                    ...
                    v-if="showAuditBtn || scope.row.status == 0 || scope.row.status == 3"
                  >编辑</el-button>
                   <el-button type="secondBtn" @click="photoExport" v-if="showAuditBtn || scope.row.status == 0 || scope.row.status == 3">资料导入</el-button>
                  <el-button
                    type="secondBtn"
    ...
    <el-dialog :visible.sync="visiDialogF" width="900px" title="资料导入">
           <div class="dialogcontent noScrollX">
        <el-scrollbar style="height:100%">
          <ul>
            <li class="liblock">
              <p class="lihead">视频导入</p>
              <div class="libody clear">
                <div class="formDivTl" style="height:100px">
                  <span>视频文件：</span>
                  <div class="layoutBox">
      ...
      <div slot="footer">
        <el-button @click="submitPhoto">保存</el-button>
        <el-button type="noticonsecondBtn" @click="cancelPhoto">取消</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    gotoEntry() {
      ...
      this.$parent.navClick({ name: "数据录入",path:`/monitorPlan/waterFuncMonitor/dataEntry/${0}`});
    },
    uploadFile(file) {
    ...
    exportReport() {
      window.open(
        "/fileXA/export/exportFuntion?keyword=" + this.searchModel + ""
      );
    },
    //提交数据
    subData() {
      ...
      this.$http
        .post("/fileXA/waterFunction/submit", qs.stringify(data))
        .then(res => {
          this.searchData();
          this.$message({
    ...
    submitReport(formName) {
      ...
      this.$http.post("/fileXA/excel/import", this.formD).then(res => {
    ...
    //修改
    revise(row) {
      this.$store.commit("setMonitordataInfo", row);
        this.$parent.navClick({ name: "数据录入",path:`/monitorPlan/waterFuncMonitor/dataEntry/${1}`});
