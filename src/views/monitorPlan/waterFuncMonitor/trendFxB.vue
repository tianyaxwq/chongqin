  ...
  <div class="singlePage secondary-page-mode">
     <div class="secondary-toolbar">
          ...
          <div class="layoutBox" style="width:100px">
             <el-select v-model="stationvalue" placeholder="请选择" multiple collapse-tags>
                   <el-option
...
export default {
  ...
  methods: {
      ...
      this.$http.get("/fileXA/waterFunction/queryWaterFunctionProjects").then(res => {
        if (res.data.code == 0) {
          ...
          for (let i in this.factorData) {
            if(i<1){
              this.factorModel=this.factorData[i].code;
            }
          }
          this.getEchartData();
        }
    ...
    getEchartData() {
      ...
      this.$http
        .get("/fileXA/waterFunction/queryMultiStations", {
          params: {
           stationNames: this.stationvalue.join(","), //站点编码（mn）'40000000000001'
            projects: this.factorModel, //因子参数（编码）'w01019'
        ...
        .then(res => {
         ...
         if(arr && arr.length>0){
            ...
            for(let i=0;i<arr.length;i++){
              this.legendData.push(arr[i].station);
