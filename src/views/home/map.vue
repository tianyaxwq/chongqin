<template>
  <div class="enterprise-map-wrap212" id="map21"></div>
</template>
...
export default {
  ...
  methods: {
    ...
    loadImageKSOv: function(val) {
      ...
      if (!this.imageOverlay_ks) {
        var imageUrl = "./static/img/sdcjgyy/1.png",
          imageBounds = [
            [22.671661756928, 113.005915435312],
            [23.02061713715, 113.387065803405]
          ];
        this.imageOverlay_ks = L.imageOverlay(imageUrl, imageBounds, {
          opacity: 0.66
        }).addTo(this.map);
      }
      ...
      this.imageKSV = setInterval(() => {
        imageOverlay_.setUrl("./static/img/sdcjgyy/" + index + ".png");
        if (index == 14) {
          index = 0;
        }
        index++;
    ...
    initAniColorYUj(svgDom, path) {
      ...
      var radialGradient = this.makeSVG("linearGradient", {
        id: idName,
        x1: "0%",
        y1: "0%",
        x2: "0%",
      ...
      var stop = this.makeSVG("stop", {
        offset: "0%",
        "stop-color": "#ade6cd",
        "stop-opacity": "1"
      });
      var stop1 = this.makeSVG("stop", {
        offset: "100%",
        "stop-color": "#79d1cf",
        "stop-opacity": "1"
      });
    ...
    markerSetup: function() {
      ...
      for (let i = 0; i < this.mapInfo.length; i++) {
        ...
        let longitude = parseFloat(stationInfo.longitude);
        let latitude = parseFloat(stationInfo.latitude);
        ...
        var text = stationInfo.stationName;
        ...
        //直观
        var aqi =
          "<span class='border-text'><i></i>" +
          (this.mapInfo[i].wqlindex ? this.mapInfo[i].wqlindex : "--") +
          (this.mapInfo[i].grade
            ? this.toWQLString(this.mapInfo[i].grade)
            : "") +
          "</span>";
        ...
        var myIcon2 = L.divIcon({
          html: "<div class='" + cls + "'>" + aqi + text + "</div>",
          className: "my-div-icon"
    ...
    airIconCreate(type, airValue) {
      var iconT = this.iconStationType2(type);
      return L.icon({
        iconUrl: "./static/img/" + iconT + ".png",
        iconSize: [26, 33]
      });
    },
    airIconCreate2(type, airValue) {
