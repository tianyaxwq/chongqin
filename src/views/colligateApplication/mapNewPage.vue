...
export default {
  ...
  methods: {
    ...
    judgeStationIcon(level, controlorLevel){
      let pre = "gk-I";
      if(controlorLevel == "1"){
        pre = "gk";
    ...
    //添加弧线
    addArcLineFn(info1, info2){
      if(info1 && info2){
        let longitude1= parseFloat(info1.lgtd);
        let latitude1 = parseFloat(info1.lttd);
        let longitude2 = parseFloat(info2.lgtd);
        let latitude2 = parseFloat(info2.lttd);
        let arcLine = new YunliMap.ArcLine({
        ...
        console.log(1, arcLine)
        this.map.add(arcLine);
        this.markers.push(arcLine);
      }
    },
    loadStationMarkers() {
      let map = this.map;
     ...
     // this.loadStationZhiguiMarkers();
      ...
      for (let i = 0; i < this.mapInfo.length; i++) {
        ...
        if(this.mapInfo.length > i+1){
          this.addArcLineFn(this.mapInfo[i], this.mapInfo[i+1]);
        }
        let longitude = parseFloat(stationInfo.lgtd);
        let latitude = parseFloat(stationInfo.lttd);
        let name = stationInfo.stnm;
        let sType = stationInfo.sttyp || "WQ08"; // 站点类型
        let namej = this.stationTypeNameJX(sType);
        let controlorLevel = stationInfo.stgrd; //控制级别
       // let icon = this.iconStationType(sType);
        let realData = stationInfo.wRealData || {};
        let wq = realData.wq_tp || "0";
        let icon =  this.judgeStationIcon(wq, controlorLevel);
      ...
      // console.log(text)
       ...
       this.markers.push(text);
        marker.on("click", function(e) {
          _this.$emit("childMethod", stationInfo.stcd);
        });
      }
    },
