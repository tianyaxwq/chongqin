      ...
      <div class="currency-mode-wrap">
        <div class="el_table_box">
          ...
          >
            ...
            <el-table-column label="湖泊编码" show-overflow-tooltip align="center">
              <template slot-scope="scope">
                <div>
                  <span v-if="scope.row.LAKE_CODE == 'null' || !scope.row.LAKE_CODE">--</span>
                  <span v-else>{{scope.row.LAKE_CODE}}</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="湖泊名称" show-overflow-tooltip align="center">
              <template slot-scope="scope">
                <div>
            ...
            <el-table-column label="总面积(平方千米)" show-overflow-tooltip align="center">
              <template slot-scope="scope">
                <div>
                  <span v-if="scope.row.AREA == 'null' || !scope.row.AREA">--</span>
                  <span v-else>{{scope.row.AREA}}</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="常年水面面积(平方千米)" show-overflow-tooltip align="center">
              <template slot-scope="scope">
                <div>
                  <span v-if="scope.row.SURAREA == 'null' || !scope.row.SURAREA">--</span>
                  <span v-else>{{scope.row.SURAREA}}</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="湖泊堤防长度(千米)" show-overflow-tooltip align="center">
              <template slot-scope="scope">
                <div>
                  <span v-if="scope.row.EMBANK_LEN == 'null' || !scope.row.EMBANK_LEN">--</span>
                  <span v-else>{{scope.row.EMBANK_LEN}}</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="所跨县" show-overflow-tooltip align="center">
              <template slot-scope="scope">
                <div>
                  <span v-if="scope.row.PASS_PR == 'null' || !scope.row.PASS_PR">--</span>
                  <span v-else>{{scope.row.PASS_PR}}</span>
                </div>
              </template>
            </el-table-column>
  ...
  methods: {
    exportExcel() {
      ...
      exportTableData(
        ...
        "",
        "1",
        "/waterEnvXA/exports/lakesBasicExcel",
        params
      );
    },
    gridFormat(row) {
    ...
    searchData() {
      this.loading = true;
      this.$http
        .get("/dataHandle/yunliBase/getDwdLake", {
          params: {
