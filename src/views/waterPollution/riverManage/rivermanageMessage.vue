      ...
      <div class="currency-mode-wrap">
        <div class="el_table_box">
          ...
          >
            ...
            <el-table-column label="河长姓名" show-overflow-tooltip align="center" prop="cmdr_nm">
              <template slot-scope="scope">
                <div>
                  <span v-if="scope.row.cmdr_nm == 'null' || !scope.row.cmdr_nm">--</span>
                  <span v-else>{{scope.row.cmdr_nm}}</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="河长职务" show-overflow-tooltip align="center" prop="cmdr_pos">
              <template slot-scope="scope">
                <div>
                  <span v-if="scope.row.cmdr_pos == 'null' || !scope.row.cmdr_pos">--</span>
                  <span v-else>{{scope.row.cmdr_pos}}</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="管辖河湖名称" show-overflow-tooltip align="center">
              <template slot-scope="scope">
                <div>
                  <span v-if="scope.row.lake_nm == 'null' || !scope.row.lake_nm">--</span>
                  <span v-else>{{scope.row.lake_nm}}</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="河长类型" show-overflow-tooltip align="center">
              <template slot-scope="scope">
                <div>
                  <span v-if="scope.row.lake_typ == '0'">总河湖长</span>
                  <span v-if="scope.row.lake_typ == '1'">河长</span>
                  <span v-if="scope.row.lake_typ == '2'">湖长</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="技术参谋姓名" show-overflow-tooltip align="center">
              <template slot-scope="scope">
                <div>
                  <span v-if="scope.row.ts_nm == 'null' || !scope.row.ts_nm">--</span>
                  <span v-else>{{scope.row.ts_nm}}</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="技术参谋单位" show-overflow-tooltip align="center">
              <template slot-scope="scope">
                <div>
                  <span v-if="scope.row.ts_unit == 'null' || !scope.row.ts_unit">--</span>
                  <span v-else>{{scope.row.ts_unit}}</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="技术参谋职务" show-overflow-tooltip align="center">
              <template slot-scope="scope">
                <div>
                  <span v-if="scope.row.ts_pos == 'null' || !scope.row.ts_pos">--</span>
                  <span v-else>{{scope.row.ts_pos}}</span>
                </div>
              </template>
            </el-table-column>
  ...
  methods: {
    exportExcel() {
      ...
      exportTableData(
        ...
        "",
        "1",
        "/waterEnvXA/exports/waterLeaderBasicExcel",
        params
      );
    },
    gridFormat(row) {
    ...
    searchData() {
      this.loading = true;
      this.$http
        .get("/dataHandle/yunliBase/getDwdUser", {
          params: {
