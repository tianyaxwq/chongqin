  ...
  <div class="curdataPage clearfix">
    <div class="currency-mode-wrap revision-table" style="height: calc(100% - 70px)">
      ...
      >
        ...
        <el-table-column prop="spt" label="监测时间" width="180">
          <template slot-scope="scope">
            <span>{{!scope.row.spt || scope.row.spt == 'null' ? '--' : scope.row.spt}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="waterQuality" label="水质类别">
          <template slot-scope="scope">
            <!-- 水质等级从0开始 -->
...
export default {
  ...
  methods: {
    ...
    searchData() {
      this.loading = true;
      this.$http
        .get("/dataHandle/yunliBase/waterRealData", {
          params: {
